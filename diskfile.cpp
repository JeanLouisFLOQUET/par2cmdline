...
# define OffsetType off_t
	...
	bool DiskFile::Create(string _filename, u64 _filesize) {
		...
		if (file == 0)                  { cerr << "Could not create: "       << _filename                     << endl; return false; }
		if (_filesize > (u64)MaxOffset) { cerr << "Requested file size for " << _filename << " is too large." << endl; return false; }
	...
	bool DiskFile::Write(u64 _offset, const void *buffer, size_t length) {
		...
		if (offset != _offset) {
			if (_offset > (u64)MaxOffset)                   { cerr << "Could not write " << (u64)length << " bytes to " << filename << " at offset " << _offset << endl; return false; }
			if (fseek(file, (OffsetType)_offset, SEEK_SET)) { cerr << "Could not write " << (u64)length << " bytes to " << filename << " at offset " << _offset << endl; return false; }
			offset = _offset;
		}

		if (length > MaxLength                              ) { cerr << "Could not write " << (u64)length << " bytes to " << filename << " at offset " << _offset << endl; return false; }
		if (1 != fwrite(buffer, (LengthType)length, 1, file)) { cerr << "Could not write " << (u64)length << " bytes to " << filename << " at offset " << _offset << endl; return false; }
	...
	bool DiskFile::Open(string _filename, u64 _filesize) {
		...
		if (_filesize > (u64)MaxOffset) { cerr << "File size for " << _filename << " is too large." << endl; return false; }
	...
	bool DiskFile::Read(u64 _offset, void *buffer, size_t length) {
		...
		if (offset != _offset) {
			if (_offset > (u64)MaxOffset                  ) { cerr << "Could not read " << (u64)length << " bytes from " << filename << " at offset " << _offset << endl; return false; }
			if (fseek(file, (OffsetType)_offset, SEEK_SET)) { cerr << "Could not read " << (u64)length << " bytes from " << filename << " at offset " << _offset << endl; return false; }
			offset = _offset;
		}

		if (length > MaxLength                             ) { cerr << "Could not read " << (u64)length << " bytes from " << filename << " at offset " << _offset << endl; return false; }
		if (1 != fread(buffer, (LengthType)length, 1, file)) { cerr << "Could not read " << (u64)length << " bytes from " << filename << " at offset " << _offset << endl; return false; }
...
bool DiskFile::Delete(void) {
	...
	if (filename.size() > 0 && 0 == _unlink(filename.c_str())) {
		return true;
	} else {
...
//==============================================================================================================================================================
// Take a filename from a PAR2 file and replace any characters which would be illegal for a file on disk
//==============================================================================================================================================================
